...
برای هر مورد، سه نوع فعالیت مرتبط از CRM در Snapshot نگه‌داری می‌شود: Note ارتباطی، Post داخلی و Task داخلی L2. هر رکورد شامل شناسه CRM، شناسه مورد، زمان ایجاد/ویرایش، متن یا عنوان، و در صورت ارائه توسط CRM شناسه و نام ثبت‌کننده است. API محلی این مجموعه را همراه Case برمی‌گرداند و رابط Case Review آن‌ها را بر اساس زمان ایجاد، از جدیدترین به قدیمی‌ترین، در یک Timeline واحد نمایش می‌دهد. نبود نام ثبت‌کننده با مقدار صریح «موجود نیست» نمایش داده می‌شود و برنامه هویت را حدس نمی‌زند.
